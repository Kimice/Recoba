...
# Print results
def print_results(hole_cards, winner_list, result_histograms, given_broad):
    ...
    for index, hole_card in enumerate(hole_cards):
        ...
        if hole_card == (None, None):
            print "(?, ?) : %.1f%%" % winning_percentage * 100
        else:
            print "%s : %.1f%%" % (hole_card, winning_percentage * 100)
    print "Ties: %.1f%%" % (float(winner_list[0]) * 100 / float_iterations)
    if given_broad and len(given_broad) == 5:
        return
    for hole_card, histogram in zip(hole_cards, result_histograms):
        if hole_card == (None, None):
            print "(?, ?) : ", " Histogram: "
        else:
            print hole_card, ": ", "Histogram:"
        for index, elem in enumerate(histogram):
            print "%s: %.2f%%" % (hand_rankings[index], float(elem) * 100 / float_iterations)
        print "-" * 20
